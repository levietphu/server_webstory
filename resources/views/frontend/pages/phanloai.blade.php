...
<nav aria-label="breadcrumb" class="pt-5px pb-5px bg-2a2b2a">
    <ol class="container breadcrumb">
        ...
        <li class="breadcrumb-item"><a class="color-white hover-a uppercase" href="">
            ...
            dưới 100 chương
            @elseif($slug_phanloai=='100-500-chuong')
            từ 100 đến 500 chương
            @elseif($slug_phanloai=='500-1000-chuong')
            từ 500-1000 chương
            @elseif($slug_phanloai=='tren-1000-chuong')
            trên 1000 chương
            @endif
            </a>
        </li>
    ...
    <div class="row row-no-gutters">
        <div class="col-12 col-md-12 col-lg-9">
            ...
            <div class="row">
                ...
                <div class="col-12 col-md-12" id="phanloai">
                ...
                <ul class="flex pl-10 pt-20">
                    ...
                    @if ($from < $i && $i <= $to)
                    <li class="mr-5px"><a class="btn btn-sm btn-outline-secondary {{$truyen->currentPage()==$i?'active':''}}" href="{{$truyen->url($i)}}#phanloai">{{$i}}</a></li>
                    @endif
                    @endfor
                    <li class="ml-20 mr-5px {{$truyen->currentPage()==$truyen->lastPage()||$truyen->currentPage()==$truyen->lastPage()-1||$truyen->currentPage()==$truyen->lastPage()-2?'display-none':''}}"><a class="btn btn-sm btn-outline-secondary uppercase" href="{{$truyen->url($truyen->lastPage())}}#phanloai">{{$truyen->lastPage()}}</a></li>
                    <li class="ml-20 mr-5px"><a class="btn btn-sm btn-outline-secondary uppercase {{$truyen->currentPage()<=1?'disabled':''}}" href="{{$truyen->previousPageUrl()}}#phanloai"><i class="fas fa-chevron-left font-size-10"></i></a></li>
                    <li class="mr-5px"><a class="btn btn-sm btn-outline-secondary uppercase {{$truyen->currentPage()>=$truyen->lastPage()?'disabled':''}}" href="{{$truyen->nextPageUrl()}}#phanloai"><i class="fas fa-chevron-right font-size-10"></i></a></li>
                </ul>
                @endif
            </div>
        </div>
