...
@section('title','Tìm truyện với từ khóa: '.$tukhoa)
@section('content')
<nav aria-label="breadcrumb" class="pt-5px pb-5px bg-2a2b2a">
    <ol class="container breadcrumb">
        ...
        <li class="breadcrumb-item"><a class="color-white hover-a uppercase" href="{{route('search')}}?tukhoa={{$tukhoa}}">Tìm truyện với từ khóa: {{$tukhoa}}</a></li>
    </ol>
</nav>
<section class="container mt-40 pb-40">
    <div class="row">
        <div class="col-12 col-md-12 col-lg-9">
            <h4>Tìm truyện với từ khóa: {{$tukhoa}}</h4>
            <div class="row">
                ...
                <ul class="flex pl-10 pt-20">
                    ...
                    @if ($from < $i && $i <= $to)
                    <li class="mr-5px"><a class="btn btn-sm btn-outline-secondary {{$truyen->currentPage()==$i?'active':''}}" href="{{$truyen->url($i)}}&tukhoa={{$tukhoa}}">{{$i}}</a></li>
                    @endif
                    @endfor
                    <li class="ml-20 mr-5px {{$truyen->currentPage()==$truyen->lastPage()||$truyen->currentPage()==$truyen->lastPage()-1||$truyen->currentPage()==$truyen->lastPage()-2?'display-none':''}}"><a class="btn btn-sm btn-outline-secondary uppercase" href="{{$truyen->url($truyen->lastPage())}}&tukhoa={{$tukhoa}}#chapter">{{$truyen->lastPage()}}</a></li>
                    <li class="ml-20 mr-5px"><a class="btn btn-sm btn-outline-secondary uppercase {{$truyen->currentPage()<=1?'disabled':''}}" href="{{$truyen->previousPageUrl()}}&tukhoa={{$tukhoa}}"><i class="fas fa-chevron-left font-size-10"></i></a></li>
                    <li class="mr-5px"><a class="btn btn-sm btn-outline-secondary uppercase {{$truyen->currentPage()>=$truyen->lastPage()?'disabled':''}}" href="{{$truyen->nextPageUrl()}}&tukhoa={{$tukhoa}}"><i class="fas fa-chevron-right font-size-10"></i></a></li>
                </ul>
                @endif
            </div>
        </div>
